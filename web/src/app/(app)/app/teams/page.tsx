    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div
          role="group"
          aria-label="View"
          className="flex items-center gap-0.5 rounded-md border border-edge bg-raised p-0.5"
        >
          <button
            type="button"
            ...
            className={cn(
              ...
              view === "list" ? "bg-accent text-black" : "text-fg-secondary hover:text-fg",
            )}
          >
            List
          </button>
          <button
            ...
            className={cn(
              "h-9 rounded px-3 text-sm font-medium transition-colors",
              view === "groups" ? "bg-accent text-black" : "text-fg-secondary hover:text-fg",
            )}
          >
            Groups
          </button>
      ...
      {teams.isPending ? (
        ...
        <StandingsTable teams={teams.data.data.items} />
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((t) => (
            <li key={t.team_id}>
              <Link
                ...
                className="flex items-center gap-3 rounded-lg border border-edge bg-surface p-3 transition-colors hover:border-edge-strong hover:bg-raised"
              >
